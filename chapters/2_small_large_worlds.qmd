...
```{r}
library(pacman)
p_load(dplyr, magrittr, ggplot2, tidyr, install = FALSE)
```
...
## Note: I'm skipping the construction of the 'forking paths' plot pastesd above
...
Next we create the 'lagged' colums (for ease of plotting the updating):
```{r}
