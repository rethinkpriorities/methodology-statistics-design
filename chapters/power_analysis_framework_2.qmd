...
The purpose of this document is to propose the fundamentals for a workflow for power analyses. Although this started from a Bayesian paradigm, the overarching framework is applicable to any analytic approach, and I will begin with a standard frequentist analysis.[^power_analysis_framework_2-2]
...
Given a particular underlying effect (or range of effects, or lack thereof), and a particular sample size (or range of sample sizes), we ask 'what is the probability of'... ?:[^power_analysis_framework_2-8]
...
```{r change-memory-limit, message = FALSE, warning = FALSE, echo=FALSE}
