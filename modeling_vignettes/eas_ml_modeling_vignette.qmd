---
title: "Vignette: Modeling donations in EA Survey data with Tidymodels and workflow"
format:
  html:
    theme: cosmo
    code-fold: true
...
[^1]: Note: not all these packages may be needed; let's search only the ones that are and drop the rest

[^2]

[^2]: another footnote in non-visual mode
...
p_load(rsample) # Why p_load these? (Can create issues with dependencies)
...
The code below reads data in directly from the RP private github repo. You need to have authorization set up for this to work.[^3]

[^3]: Note this data was already edited in an analysis file -- we should move that over to one of the R 'build files'
...
Normally, we should do the main data manipulation steps separately from the analysis.[^4] However, if there are processing steps that are very specific to a modeling exercise, it seems OK to do them at this stage; but we should remain vigilant.[^5] <!-- I think it may be worth expanding on this. In that that we want to perform processing steps which are going to be long and will not vary during modelling. But where we might wish to (for example) impute data we may wish to include this as a part of the modelling process to allow testing of various imputation methods. Also things like normalizing variables should obviously be done as part of the modelling stage because of data leakage. -->

[^4]: In the present case this would be in `build/eas_cross_year_harmonisation.R`.

[^5]: Furthermore, data-based imputation and standardizing variables must be done *later,* at the modeling stage; we do this with the `recipe` package below.

First we remove columns with only missing values, to clean up what we have to consider, and because this may mess up processing.[^6]

[^6]: However, this is a case where this 'might as well have been done at the 'build the data' stage ... so I'll move it there.
...
Next, another 'Big global filtering step' ... .[^7], [^8]

[^7]: This should remove any people who did not answer the donation question... on which (all our) outcome variables are based. But actually, something is weird here -- it removes very few observations, so I'm not sure what is going on, need to doublecheck it

[^8]: Of course, as the decision to answer the donation question is nonrandom, this is not uncontroversial. There may be some selection issues here. Alternate models could consider, e.g., 1. Non-responses as zeroes, 2. The binary outcome 'reported a positive donation', 3. Some explicit selection model.
...
Next, I define an 'optional filter', which we may use below.[^9] `income_filter` will remove individuals with an income below 500000.[^10]

[^9]: I define this 'filter object' using fancy 'quosure' syntax here, rather than actually creating another slice of the data as an object. This is in part to avoid clutter.

[^10]: `income_c_imp_bc5k` actually does a particular imputation for missing or near-zero incomes. I use this imputed variable here for consistency, but it is not important here, as we are removing the *highest* income people only.
...
Finally, for factor variables, we set base categories to the most common values.[^11]

[^11]: This could have been done in the building process, but different procedures and reporting may want to use these categories differently, so it's OK to do it in the moment.
...
For ML modeling, we need to set aside some data to test our model's ultimate performance. We thus 'split' our data sets using the `rsample` package tools. 3/4 of data is used for 'training' (fitting the parameters of our model), and the rest for testing.[^12]

[^12]: Why this particular split? I'm not sure, maybe it's a rule of thumb.
...
I next create the 'non-highest-income' subset of the data, using the `income_filter` quosure object defined above to the testing and training data.[^13]

[^13]: Consider: in some cases we might want to filter *before* making training/test, but there are pros and cons
...
In the full model we compare both the unfiltered and filtered data. For simplicity, I'll use only the latter here.[^14]

[^14]: Why this filter? I suspected that our methods may be sensitive to outliers, particularly with some of the functional forms. I could instead try to adjust the *method* of course, considering the real criteria of interest. This may be just an interim solution.

## Create partitions (folds) for validation 'tuning' step

```{=html}
<!--
(Perhaps relevant to below discussion instead)
- Testing the performance of different parameter combinations on the test set rather than validation set is *cheating* and a bit leaky
-->
```
```{r}
...
This is iterated many times through different regularization specifications (hyper-parameters). The cross-validation results allow one to estimate the level of regularization which provides the best out-of-sample predictive performance.[^15]
:::

[^15]: This may not always be necessary. There are some claims that \[Oska ... ref?\]
...
The `rsample::vfold_cv` function does this resampling (with a default of ten folds).[^16]

[^16]: Why ten? Maybe that's a conventional rule of thumb? *Oska:* This seems to be standard in ML, in cases where leave-one-out cross-validation isn't feasible (which is most of the time).
...
Each of these particions classes 90% of the data for 'analysis' and about 10% for 'assessment'.

# Defining and creating the formula objects ('equations' to model)

First I make a list of 'right hand side variables' here,[^17] the columns being used in prediction, to shorten later code, avoiding duplication, and centralize things.[^18]
...
[^18]: I know I said this is good to do 'at the top', but this is the part where we are starting to actually get into the modeling, so it seems OK.
...
Next we create three formula objects,[^19] each with the same set of rhs variables, but a different 'outcome' variable.[^20]

[^19]: It might be more tidy to do this imputting a list of outcome variables too, and generating a list of formulas to use later. Leaving this as a 'to do'.

[^20]: `make_formula` uses `stats::reformulate` to collapse lists into a formula object.
...
As noted, in a machine learning context, especially if we care only about prediction for its own sake, 'more features (columns) always help' Thus, we try to fit a model that allows many, many variables, imputing these where there are missing values.[^21] This should get us the model that 'predicts best out of sample'. However we *need to be very careful in interpreting these coefficients*.

[^21]: Imputing them based on non-leaky variables only, of course, not based on any post-outcome measures.

For ML procedures to work, we often always need to standardize each continuous (predictor) column, subtracting its mean and dividing by it's estimated standard deviation.[^22]

[^22]: This is basically because we need to 'penalize each coefficient fairly'. Note that we can also divide by any scalar for interpretation, as long as we do this the same for all columns. This comes up later. (This may only be the case for regression-based approaches.) Oska: "Really depends on the type of model, for very popular tree-based models standardization isn't required."
