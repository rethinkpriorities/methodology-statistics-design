...
[^eas_ml_modeling_vignette-2]

[^eas_ml_modeling_vignette-2]: another footnote in non-visual mode
...
First we remove columns with only missing values, to clean up what we have to consider, and because this may mess up processing.[^eas_ml_modeling_vignette-6]
...
[^eas_ml_modeling_vignette-9]: I define this 'filter object' using fancy 'quosure' syntax here, rather than actually creating another slice of the data as an object. This is in part to avoid clutter.
...
[^eas_ml_modeling_vignette-11]: This could have been done in the building process, but different procedures and reporting may want to use these categories differently, so it's OK to do it in the moment.
...
In the full model we compare both the unfiltered and filtered data. For simplicity, I'll use only the latter here.[^eas_ml_modeling_vignette-14]
...
First I make a list of 'right hand side variables' here,[^eas_ml_modeling_vignette-17] the columns being used in prediction, to shorten later code, avoiding duplication, and centralize things.[^eas_ml_modeling_vignette-18]
...
[^eas_ml_modeling_vignette-19]: It might be more tidy to do this imputting a list of outcome variables too, and generating a list of formulas to use later. Leaving this as a 'to do'.
...
For ML procedures to work, we often always need to standardize each continuous (predictor) column, subtracting its mean and dividing by it's estimated standard deviation.[^eas_ml_modeling_vignette-22]
